...
public class ScoreService {
    ...
    public ScoreDTO.Response calculateAndSaveScore(ScoreDTO.Request scoreRequest, HttpServletRequest request) {
        ...
        long endTime = (long) session.getAttribute("questionEndTime");
        ...
        // 4. (⭐️핵심⭐️) User의 총점(hapScore)을 업데이트합니다.
        User saveUser = userRepository.findById(currentUser.getId()).get();
        int currentHapScore = saveUser.getHapScore() == null ? 0 : saveUser.getHapScore();
        saveUser.setHapScore(currentHapScore + calculatedScore);
        // userRepository.save(currentUser); // currentUser는 현재 영속성 컨텍스트에 있으므로, @Transactional에 의해 자동으로 변경 감지(dirty checking) 되어 저장됩니다.
